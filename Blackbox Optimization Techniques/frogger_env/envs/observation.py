...
class Neighborhood(ObservationType):
    ...
    def normalize_obs(self, obs):
        ...
        features_range = [
                ...
                [-self.env.road.speed - self.env.agent.speed, self.env.road.speed + self.env.agent.speed], # vx
                [-self.env.agent.speed, self.env.agent.speed]] #vy
        for o in obs:
            for i, feature in enumerate(o):
                n_obs.append( (feature - features_range[i][0])*1./(features_range[i][1] - features_range[i][0]))
        return n_obs

    def space(self) -> spaces.Space:
        return spaces.Box(shape=[self.nearest_neighbor*4], low=-1, #self.max_distance,
                          high=1, dtype=np.float32) #self.max_distance, dtype=np.float32)

    def observe(self) -> np.ndarray:
        agent_pos = self.env.agent.position
        vehicles = self.env.road.vehicles
        
        '''vehicles_pos = [v.position for v in vehicles]
        dist = np.linalg.norm(agent_pos - vehicles_pos, axis=1)
        knn_idx = np.where(dist <= self.max_distance)[0]
        closest_vehicles = [vehicles[tuple(k)] for k in knn_idx]'''
        ...
        if closest_vehicles:
            ...
            closest_vehicles = closest_vehicles[:5]
        ...
        for _ in range(self.nearest_neighbor - len(closest_vehicles)):
            observation.append([self.max_distance,
                                self.max_distance, self.env.road.speed + self.env.agent.speed, self.env.agent.speed])
                                #self.env.road.speed, -self.env.agent.velocity[1]])
...
class Position(ObservationType):
    def __init__(self, env, normalize = True):
        super().__init__(env)
        self.origin = np.array(self.env.observation_config["origin"])
        self.normalize = normalize
        if normalize:
            self.normalizer = np.array([self.env.config["world_bounds"][2], self.env.config["world_bounds"][3]]) #- self.origin
        else: 
            self.normalizer = np.array([1.,1.])
        self.augment = None
        if self.env.observation_config["augment"] == "occupancy_grid":
            self.augment = OccupancyGrid(self.env)
        elif self.env.observation_config["augment"] == "lidar":
            self.augment = LidarScan(self.env)
        elif self.env.observation_config["augment"] == "neighborhood":
            self.augment = Neighborhood(self.env)

    def space(self) -> spaces.Space:
        if not self.augment:
            return spaces.Box(shape=[4], low=0,
                              high=1, dtype=np.float32)
        elif self.augment.type == "occupancy_grid":
            box = self.augment.space()
            return spaces.Box(shape=[4 + box.shape[0] * box.shape[1]], low=0,
                              high=1, dtype=np.float32)
        elif self.augment.type == "lidar":
            box = self.augment.space()
            return spaces.Box(shape=[4 + box.shape[0]], low=0,
                              high=1, dtype=np.float32)
        elif self.augment.type == "neighborhood":
            box = self.augment.space()
            return spaces.Box(shape=[4 + box.shape[0]], low=0,
                              high=1, dtype=np.float32)

    def observe(self) -> np.ndarray:
        agent_pos = (self.env.agent.position - self.origin)/self.normalizer
        goal_pos = (self.env.goal - self.origin)/self.normalizer
        obs = np.concatenate([agent_pos, goal_pos])
        #obs = np.array(agent_pos)
        if self.augment:
            augmented = self.augment.observe()
            if len(np.shape(augmented)) > 1:
                augmented = augmented.flatten()
            obs = np.concatenate([obs, augmented])
